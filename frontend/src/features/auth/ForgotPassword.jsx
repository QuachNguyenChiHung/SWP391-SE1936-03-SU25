...
export const ForgotPassword = () => {
  ...
  const handleSend = async (e) => {
    ...
    setIsLoading(true);
    try {
      // Call backend endpoint to request forgot-password email
      const res = await api.post('/Auth/forgot-password', { email });

      // If backend returns an OTP (for client-side verification), store it and show OTP UI
      if (res?.data?.otp) {
        const otpCode = String(res.data.otp).slice(0, 6);
        ...
        // Otherwise, backend handled emailing (common case). Inform user to check email.
        setOtpSent(false);
        setStatus({ type: 'success', message: res?.data?.message || 'Check your email for reset instructions.' });
      }
    } catch (err) {
      console.error('Backend forgot-password failed, falling back to emailjs:', err);
      // Fallback to client-side EmailJS if backend call fails
      try {
