...
const GuidelinesPanel = ({ guidelines }) => {
    ...
    const guidelineItems = Array.isArray(guidelines) ? guidelines : 
                          (typeof guidelines === 'string' && guidelines ? [guidelines] : []);
        ...
        <div className="bg-white border rounded shadow-sm d-flex flex-column" style={{ flex: '0 0 auto' }}>
            ...
            <div className="p-4 bg-white">
                {guidelineItems.length > 0 ? (
                    <ul className="list-unstyled mb-0" style={{ fontSize: '12px', lineHeight: '1.6' }}>
                        {guidelineItems.map((item, index) => (
                            <li key={index} className="text-muted mb-2 d-flex">
                    ...
                    <div className="text-muted" style={{ fontSize: '12px', lineHeight: '1.6' }}>
                        No specific guidelines set for this project.
                    </div>
                )}
            </div>
        </div>
...
GuidelinesPanel.propTypes = {
    guidelines: PropTypes.oneOfType([
        PropTypes.string,
