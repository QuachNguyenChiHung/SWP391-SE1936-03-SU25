...
export const AnnotatorWorkspace = ({ user }) => {
        ...
        const fetchBatchItems = async () => {
            ...
            try {
                ...
                if (itemIdFromUrl) {
                    ...
                    if (itemToSelect) {
                        // Use handleSelectItem to properly load annotations
                        await handleSelectItem(itemToSelect);
                    } else {
                        setSelectedItem(null);
                    }
                } else {
        ...
        try {
            ...
            if (newAnnotation) {
                ...
                const transformedAnnotation = {
                    ...
                    labelName: newAnnotation.labelName || 'Unknown',
                    labelColor: newAnnotation.labelColor || '#6366f1',
                    coordinates: processedCoordinates,
                ...
                console.log('Label name:', transformedAnnotation.labelName, 'Label color:', transformedAnnotation.labelColor);
    ...
    const handleDeleteAnnotation = async (annotationId) => {
        ...
        if (selectedBatch?.status === 'Submitted' || 
            ...
            selectedItem?.dataItemStatus === 'Approved') {
            showToast('Cannot delete annotation: Task has been submitted or item is approved', 'warning');
            return;
        }
    ...
    const handleAcceptAndNext = async () => {
        ...
        // Skip if already completed, submitted, or approved - just navigate
    ...
    const handleSubmitTask = async (taskId) => {
        ...
        // Check if all items are completed (or already submitted/approved)
        ...
        try {
            ...
            console.log('Submit task response:', response.data);

            const nowIso = new Date().toISOString();

            // Refresh the task items to get updated statuses from backend
            try {
                ...
                console.error('Failed to refresh items after submit:', refreshError);
                showToast('Failed to refresh items: ' + (refreshError?.response?.data?.message || refreshError?.message), 'error');
            }
        } catch (e) {
            ...
            if (isDrawing && drawingStartRef.current && currentDragInfo) {
                ...
                if (w > 5 && h > 5) {
                    ...
                    if (img) {
                        ...
                        const x1Percent = (x / imgWidth) * 100;
                        const y1Percent = (y / imgHeight) * 100;
                        const x2Percent = ((x + w) / imgWidth) * 100;
                        const y2Percent = ((y + h) / imgHeight) * 100;

                        // Create bbox coordinates as percentages
                        const coordinates = [
                            { x: x1Percent, y: y1Percent },
                            { x: x2Percent, y: y2Percent }
                        ];
    ...
    const handleAnnotationMouseDown = (e, ann) => {
        // Prevent dragging if task is submitted or item is approved
        if (selectedBatch?.status === 'Submitted' || 
            selectedBatch?.status === 'Completed' ||
            selectedItem?.dataItemStatus === 'Submitted' ||
            selectedItem?.dataItemStatus === 'Approved') {
    ...
    const handleContainerMouseDown = (e) => {
        ...
        // Prevent drawing if task is submitted or item is approved
        if ((selectedBatch?.status === 'Submitted' || 
             selectedBatch?.status === 'Completed' ||
             selectedItem?.dataItemStatus === 'Submitted' ||
             selectedItem?.dataItemStatus === 'Approved') && 
    ...
    const handleContainerDoubleClick = (e) => {
        if (selectedTool === 'POLYGON' && polygonPoints.length >= 3 && 
            ...
            if (img) {
                ...
                const percentagePoints = polygonPoints.map(p => ({
                    x: (p.x / imgWidth) * 100,
                    y: (p.y / imgHeight) * 100
                }));
                        ...
                        >
                            <div className={`canvas-image-frame canvas-item-scene ${itemTransitionPhase !== 'idle' ? `scene-${itemTransitionPhase}` : ''}`} style={{ position: 'relative', display: 'inline-block' }}>
                                ...
                                {showGuidelines && annotations.map((ann) => {
                                    ...
                                    // Use NATURAL dimensions to match how coordinates are saved
                                    const img = imageRef.current;
                                    const imgWidth = img?.naturalWidth || img?.width || 1;
                                    const imgHeight = img?.naturalHeight || img?.height || 1;
                                    
                                    // Debug log for first annotation
                                    if (ann.id === annotations[0]?.id) {
                                        console.log('ANNOTATOR - First annotation:', {
                                            id: ann.id,
                                    ...
                                    if (ann.coordinates.type === 'bbox' && ann.coordinates.points && ann.coordinates.points.length === 2) {
                                        ...
                                        if (ann.id === annotations[0]?.id) {
                                            console.log('ANNOTATOR - First bbox calculation:', {
                                                p1, p2,
                                                x1, y1, x2, y2,
                                                width, height
                                            });
                                    ...
                                    if (ann.coordinates.type === 'polygon' && ann.coordinates.points) {
                                        // Coordinates are in percentages, convert to pixels
                                        const points = ann.coordinates.points.map(p => ({
                                            x: p.x * imgWidth / 100,
                                            y: p.y * imgHeight / 100
                                        }));
                    ...
                    <div className="annotator-action-bar p-4 bg-white border-top border-slate-200">
                        ...
                        <div className="d-flex align-items-center gap-3" style={{ height: '3rem' }}>
                            ...
                            {(selectedItem?.dataItemStatus === 'Reported' || selectedItem?.dataItemStatus === 8 || selectedItem?.dataItemStatus === '8') && 
                             selectedItem?.dataItemStatus !== 9 && selectedItem?.dataItemStatus !== '9' && selectedItem?.dataItemStatus !== 'Resolved' ? (
                                ...
                                <button
                                    ...
                                    title={
                                        selectedBatch?.status === 'Submitted' || selectedItem?.dataItemStatus === 'Submitted' || selectedItem?.dataItemStatus === 'Approved'
                                            ? 'Cannot flag submitted or approved items'
                                            : t.flagItemTooltip
                                    }
                                >
                                    <AlertTriangle size={18} />
                            ...
                            <button
                                ...
                                disabled={((selectedItem?.dataItemStatus === 'Reported' || selectedItem?.dataItemStatus === 8 || selectedItem?.dataItemStatus === '8') && selectedItem?.dataItemStatus !== 9 && selectedItem?.dataItemStatus !== '9' && selectedItem?.dataItemStatus !== 'Resolved')}
                                ...
                                style={{ 
                                    fontSize: '0.875rem', 
                                    opacity: ((selectedItem?.dataItemStatus === 'Reported' || selectedItem?.dataItemStatus === 8 || selectedItem?.dataItemStatus === '8') && selectedItem?.dataItemStatus !== 9 && selectedItem?.dataItemStatus !== '9' && selectedItem?.dataItemStatus !== 'Resolved') ? 0.5 : 1, 
                                    cursor: ((selectedItem?.dataItemStatus === 'Reported' || selectedItem?.dataItemStatus === 8 || selectedItem?.dataItemStatus === '8') && selectedItem?.dataItemStatus !== 9 && selectedItem?.dataItemStatus !== '9' && selectedItem?.dataItemStatus !== 'Resolved') ? 'not-allowed' : 'pointer'
                                }}
                                title={
                                    selectedItem?.dataItemStatus === 'Submitted' || 
                                    selectedItem?.dataItemStatus === 'Approved' || 
