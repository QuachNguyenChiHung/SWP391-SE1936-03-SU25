...
import {
    Plus, MoreHorizontal, Calendar, Tag, Layers, FileText,
    ChevronRight, Filter, ArrowLeft, Pencil, Trash2, ChevronDown, ChevronUp,
    Upload, X, Image as ImageIcon, Save, PieChart as PieChartIcon, UserPlus, CheckCircle
} from 'lucide-react';
...
export const ManagerProjects = ({ user }) => {
    ...
    // Load selected project from URL parameter
    useEffect(() => {
        if (id) {
    ...
    const handleImport = () => {
        ...
        const interval = setInterval(() => {
            ...
            if (progress >= 100) {
                ...
                selectedProject.totalItems += newTasks.length;
    ...
    const openGuidelines = () => {
        if (selectedProject) {
            setGuidelinesText(selectedProject.guidelines || '');
    ...
    const handleSaveGuidelines = () => {
        if (selectedProject) {
            selectedProject.guidelines = guidelinesText;
            setIsEditingGuidelines(false);
        }
    };
        ...
        return (
            <div className="h-100 d-flex flex-column gap-4 animate-in fade-in slide-in-from-right-4 duration-300 position-relative">
                <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h2 className="h5 fw-bold text-dark mb-1">{selectedProject.name}</h2>
                        <div className="d-flex align-items-center gap-3">
                            <p className="small text-muted mb-0">{selectedProject.type.replace(/_/g, ' ')}</p>
                            <span className={`badge ${selectedProject.priority === 'HIGH' ? 'bg-danger-light text-danger' :
                                selectedProject.priority === 'MEDIUM' ? 'bg-orange-light text-warning' :
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="d-flex align-items-center justify-content-center" style={{ height: '16rem' }}>
                                            <ResponsiveContainer width="100%" height="100%">
                                                <PieChart>
                                                    <Pie
                                                        data={[
                                                            { name: 'Completed', value: selectedProject.completedItems },
                                                            { name: 'Remaining', value: selectedProject.totalItems - selectedProject.completedItems }
                                                        ]}
                                                        innerRadius={60}
                                                        outerRadius={80}
                                                        paddingAngle={5}
                                        ...
                                        <div className="d-flex justify-content-center gap-5 mt-4">
                                            <div className="text-center">
                                                <p className="h3 fw-bold mb-1">{selectedProject.completedItems}</p>
                                                <p className="small text-muted text-uppercase">Done</p>
                                            </div>
                                            <div className="text-center">
                                                <p className="h3 fw-bold mb-1">{selectedProject.totalItems}</p>
    ...
    return (
        <div className="h-100 d-flex flex-column gap-4 animate-in fade-in duration-300">
            <div className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3">
                ...
                <button className="btn btn-primary d-flex align-items-center gap-2 w-100 w-sm-auto justify-content-center">
                    <Plus size={16} />
                    Create Project
                </button>
            </div>
                    ...
                    <div key={project.id} className="col-12 col-md-6 col-xl-4">
                        ...
                        >
                            <div className="card-body">
                                ...
                                <p className="small text-muted mb-3" style={{ minHeight: '2.5rem', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{project.description}</p>

                                <div className="mb-3">
                                    <div className="d-flex justify-content-between small text-muted mb-2">
                                        <span>Progress</span>
                                        <span className="fw-medium text-dark">{Math.round((project.completedItems / project.totalItems) * 100)}%</span>
                                    </div>
                                    <div className="progress" style={{ height: '6px' }}>
                                        <div
                                            className="progress-bar bg-primary"
                                            style={{ width: `${(project.completedItems / project.totalItems) * 100}%`, transition: 'width 1s' }}
                                        ></div>
                                    </div>
                                </div>
