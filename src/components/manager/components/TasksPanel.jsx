...
export default function TasksPanel({ tasksByAssignee, expandedTaskGroups, toggleGroup, StatusBadge }) {
    ...
    const fetchTasks = async (pageNumber = 1, pageSize = 10, projectIdProp) => {
        ...
        try {
            ...
            // build annotators list from items
            const annotatorMap = {};
            items.forEach(it => {
                const aId = it.annotatorId ?? 0;
                const aName = it.annotatorName || (aId === 0 ? 'Unassigned' : `User ${aId}`);
                if (!annotatorMap[aId]) annotatorMap[aId] = { id: aId, name: aName, activeTaskCount: 0 };
                annotatorMap[aId].activeTaskCount += (it.totalItems || 1);
            });
            setAnnotators(Object.values(annotatorMap));
        } catch (e) {
    ...
    return (
        <div className="d-flex flex-column gap-3">
            ...
            {annotators.map((assignee) => {
                const tasks = tasksByAnnotator[assignee.id] || tasksByAssignee[assignee.id] || [];
                ...
                return (
                    <div key={assignee.id} className="card border-0 shadow-sm overflow-hidden">
                            ...
                            <div className="d-flex align-items-center gap-3">
                                ...
                                <div className="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <small className="text-muted">{tasks.length} items{assignee && ` • ${assignee.activeTaskCount} active`}</small>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <img src={buildUploadsUrl(item.thumbnailPath || item.filePath)} alt="thumb" style={{ width: 64, height: 48, objectFit: 'cover' }} />
...
export function MockTasksPanel({ tasksByAssignee, expandedTaskGroups, toggleGroup, StatusBadge }) {
    const [annotators, setAnnotators] = useState([]);
    const [showAssignModal, setShowAssignModal] = useState(false);
    const [selectedAssignee, setSelectedAssignee] = useState(null);
    const [selectedDataItemIds, setSelectedDataItemIds] = useState([]);
